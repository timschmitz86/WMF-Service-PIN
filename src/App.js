...
function App() {
  ...
  const fetchPinData = async () => {
    try {
      // Use REACT_APP_API_URL when provided (strip trailing slash), otherwise use relative /api
      const apiBase = process.env.REACT_APP_API_URL ? String(process.env.REACT_APP_API_URL).replace(/\/$/, '') : '';
      const response = await fetch(`${apiBase}/api/pin`);
